...
4) select book_name, count(book_name) as count from get_transactions_in_given_month(6,2017)
...
7) select user_id, count(user_id) from get_transactions_in_given_month(7,2017)
...
12)-select avg(returned_on - borrowed_on) avg_of_time_of_holding_book from
